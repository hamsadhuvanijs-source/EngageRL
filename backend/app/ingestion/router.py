...
async def add_file_source(chat_id: str, file: UploadFile, db: Session = Depends(get_db)) -> MaterialSourceOut:
...
def add_link_source(chat_id: str, body: LinkSourceIn, db: Session = Depends(get_db)) -> MaterialSourceOut:
    chat = _get_chat(db, chat_id)
...
def add_text_source(chat_id: str, body: TextSourceIn, db: Session = Depends(get_db)) -> MaterialSourceOut:
    chat = _get_chat(db, chat_id)
...
def delete_source(chat_id: str, source_id: str, db: Session = Depends(get_db)) -> None:
    chat = _get_chat(db, chat_id)
